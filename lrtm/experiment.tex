...
\cref{alg:lr-jsmf}, learning the compressed statistics \smash{$Y_{LR-JSMF}$}
again by running ENN initialized with \smash{$V \sqrt{D}$}. Then we run the
...
\textbf{Specificity} \smash{$\frac{1}{K} \sum_k \text{KL}(B_{\ast k}||\sum_iC_
{\ast i})$} indicates that the learned clusters are different enough from the
...
\textbf{Sparsity} \smash{$\left(\frac{1}{K} \sum_k \frac{\sqrt{N} - (\|b_k\|_1 /
\|b_k\|_2)}{\sqrt{N} - 1}\right)$} \cite{Hoyer2004} says that our topic clusters
are
